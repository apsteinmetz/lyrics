...
get_ngrams <- function(lyric_df,n){
  lyric_ngram <- ramones_lyrics %>% 
    ...
    count() %>% 
    arrange(desc(n)) %>% 
    filter(n>1) %>% mutate(want=str_remove(ngram,want_phrases)) %>% 
    rownames_to_column()
  return(lyric_ngram)
...
ultimate_want <- bind_rows(ultimate_want,want[c(2,4,8,9,11),])
...
```
```{r}
ultimate_want <- bind_rows(ultimate_want,want[c(8,13),])
want <- ramones_lyrics %>% get_ngrams(7)
wantultimate_want <- bind_rows(ultimate_want,want[c(4),])
