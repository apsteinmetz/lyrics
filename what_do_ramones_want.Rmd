...
I am not happy with this function.  It relies on the lyrics always being the 22nd `<div>` tag on the page.  A better way would identify smartly which block of the page contained the lyrics.  The code shows in a commented line one attempt at this.  It didn't work.
```{r}
...
```
This is a sample of what we get.
```{r}
print(all_lyrics[1])
```

It's just a text string.  Before we can use `tidytext` we'll make the list of lyric strings into a complete data frame with all the trimmings.
```{r}
