...
But is it accurate?  You may accuse me of being that pedantic "Comic Book Guy" from "The Simpsons" but, when I saw it, I immediately wondered how I could tally these Ramones lyrics myself.
...
Now lets find what the Ramones Want.  Let's look at the most common igrams, which would include the phrases like "I want" and "I wanna."
...
```{r}
want_phrases <- "^(i wanna |i want |we want |we wanna |i wanted )"
...
ultimate_want <- bind_rows(ultimate_want,want[c(2,4,8,11),])
...
```{r}
ultimate_want <- bind_rows(ultimate_want,want[c(1,3,22,23),])
...
```{r}
ultimate_want <- bind_rows(ultimate_want,want[c(6,13,18),])
want <- ramones_lyrics %>% get_ngrams(6)
want

```
```{r}
ultimate_want <- bind_rows(ultimate_want,want[c(3,8,13),])
want <- ramones_lyrics %>% get_ngrams(7)
want
```

```{r}
ultimate_want <- bind_rows(ultimate_want,want[c(4),])
ultimate_want <- ultimate_want %>% arrange(desc(n)) %>% select(-rowname)
ultimate_want
```
Now strip "to" so that everthing sounds like it comes after "wanna" instead of "want" and do some other clean up.
```{r}
ultimate_want <- ultimate_want %>% mutate(want = str_remove(want,"to |ed |wanna ")) %>% 
  group_by(want) %>%
  summarise(n=sum(n))
ultimate_want
```
...
```{r}
library(scales)
ultimate_want %>% filter(sentiment=="want") %>% 
  mutate(want=reorder(want,n)) %>% 
  ggplot(aes("",n,fill=want)) + geom_col()+ 
